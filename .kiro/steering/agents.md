...
These are the leading words for all 16 skills. In recognition mode, these are enough. For depth, load the full skill from `.kiro/skills/internal/<skill-name>/SKILL.md` (lowercase-hyphenated folder names).
...
| Challenge from user value lens | `pm-requirements-challenger` | competitor test, journey, would they pay |
| Destroy weak decisions | `requirements-red-team` | 5 whys, contradict, untestable, over-engineering |
| Scan architecture for friction | `improve-codebase-architecture` | shallow modules, deepening, testability, friction |
...
1. Check if their assigned skill has a `SKILL.md` at `.kiro/skills/internal/<skill-name>/SKILL.md`
...
- **Requirements review:** `requirements-red-team` attacks for contradictions/complexity + `pm-requirements-challenger` attacks from user-value lens. Both MUST review before requirements are approved.
- **Architecture review:** `improve-codebase-architecture` scans for friction and deepening opportunities
...
| Requirements by `pm-lead` | `requirements-red-team` (destroy weak decisions) + `pm-requirements-challenger` (user-value lens) |
| Design decisions | `pm-requirements-challenger` validates user value |
| Architecture | `improve-codebase-architecture` scans for friction |
