...
export default function ReadyToGoSection() {
    ...
    useEffect(() => {
        const fetchEvents = async () => {
            try {
                const response = await eventsApi.getAll({ todayOnly: 'true', limit: '4' }) as { events: Event[] };
                        ...
                        <div className="relative z-10">
                                        ...
                                        return (
                                            <StaggerItem key={event._id}>
                                                <Link href={`/events/${event._id}`}>
                                                    <div className="glass-card overflow-hidden group cursor-pointer h-full hover:-translate-y-1 transition-transform duration-300 border border-emerald-500/20">
                                                        {/* Image */}
                                                        <div className="relative h-40 overflow-hidden">
                                                            ...
                                                            {/* Time Badge - Urgent style */}
                                                            <div className="absolute top-3 left-3 bg-emerald-500/90 backdrop-blur-sm px-2 py-1 rounded-full">
                                                                <span className="text-white text-xs font-semibold">{formatRelativeTime(event.startDateTime)}</span>
                                                            </div>
                                                        ...
                                                        {/* Content */}
                                                        <div className="p-4">
                                                            <h3 className="text-sm font-medium text-white mb-1 line-clamp-1 group-hover:text-emerald-400 transition-colors">
                                                                {event.name}
                                                            </h3>
                            ...
                            <SlideUp delay={0.3}>
                                <div className="text-center mt-8 sm:hidden">
                                    <Link
                                        href="/events/ready-to-go"
                                        className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/20 transition-colors"
                                    >
                                        See all events
                                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M17 8l4 4m0 0l-4 4m4-4H3" />
