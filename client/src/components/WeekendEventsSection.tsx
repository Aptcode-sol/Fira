...
export default function WeekendEventsSection({ cityFilter }: WeekendEventsSectionProps) {
    ...
    const fetchEvents = useCallback(async () => {
        setIsLoading(true);
        try {
            const params: Record<string, string> = { weekend: 'true', limit: '10' };
    ...
    const seeAllHref = cityFilter
        ? `/events?weekend=true&city=${encodeURIComponent(cityFilter)}`
        : '/events?weekend=true';
