...
import { BackButton, ShareButton, Button, Modal } from '@/components/ui';
...
/**
 * `initialVenue` is the document the server already read for this page's metadata and
 * Place schema. Seeding from it is what puts the venue's name, city, capacity and
 * price into the HTML a crawler receives - see the note in page.tsx.
 */
export default function VenueDetailClient({ initialVenue = null }: { initialVenue?: Venue | null }) {
    ...
    const fetchVenue = async (id: string) => {
        try {
            // No setIsLoading(true) here: when the server seeded the venue, flipping
            // back to loading would discard the rendered page and flash the skeleton
            // on every mount.
            const data = await venuesApi.getById(id);
            setVenue(data as Venue);
        } catch (error) {
            console.error('Failed to fetch venue:', error);
            // Only fall back to mock data when there is nothing rendered - never
            // replace a real venue because a background refresh failed.
            setVenue(prev => prev ?? getMockVenue(id));
        } finally {
            setIsLoading(false);
        }
    };
    ...
    // Shared formatter: this page renders the booking checkout breakdown, so the
    // paise it shows have to be the paise the gateway charges.
                            ...
                            {(() => {
                                ...
                                return (
                                    <div className="space-y-2 text-sm">
                                        ...
                                        {/* Amount only - the commission rate is not published
                                            at checkout. Same as BillingCard. */}
                                        <div className="flex justify-between gap-3 text-gray-300">
